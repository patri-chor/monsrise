...
async function runTests() {
  ...
  try {
    ...
    // Test 5: 验证端到端流水线完全限定在 testTmpDir 输出 (T013-5)
    console.log('[Test 5] 验证流水线在测试临时目录 tests/.tmp/per-seed-expansion/ 隔离运行...');
    ...
    // Test 6: 验证所有受保护生产文件 byte-identical 零污染 (T013-6)
    console.log('[Test 6] 验证所有受保护生产文件 byte-identical 零污染...');
    ...
    console.log('=== 所有 T013 验收测试全部通过 (6/6) ===');
  } finally {
    if (existsSync(testTmpDir)) {
      rmSync(testTmpDir, { recursive: true, force: true });
    }
