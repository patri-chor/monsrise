...
import { recognizeArchetype, type FeatureMask, type EvolFormation, type EvolNode, cloneEvolFormation, emptyMask, walkEvolNodes } from '../evol_gene';
import { computeProductOutcomeFromMatch, compareProductOutcome, aggregateProductOutcomes, type ProductOutcome } from './tree_product';
import type { RoundBoardEdit } from '../round_board';
...
export interface PairedValidation {
  ...
  classification: 'IMPROVES' | 'REGRESSES' | 'NEUTRAL' | 'NOT_SELECTED';
}
...
export class TreeLineage {
  ...
  public static compileForwardCandidate(
    ...
    const hasReposition = rep.edits.some((e: any) => e.type === 'REPOSITION_DEPLOYED_UNIT');
    ...
    const targetPlacements = adverseCase.baseState.pendingActions
      ...
      .map((a: any) => {
        const edit = rep.edits.find((e: any) => e.type === 'CHANGE_PENDING_PLACEMENT' && e.actionOrder === a.order);
  ...
  public static validateCandidateAgainstCurrentPilot(
    ...
    const candEvol = TreeLineage.attachExecutableBranchesToEvol(currentPilotEvol, [cand.executableBranch]);
