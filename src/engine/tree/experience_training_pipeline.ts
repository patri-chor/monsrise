...
/** FNV-1a makes resume identity depend on the actual evaluated content. */
export function fingerprintExperienceValue(value: unknown): string {
...
/** Candidate IDs are labels; resume identity must include the evaluated deck and tree. */
export function getCandidateObservationFingerprint(candidate: { team: unknown; tree: unknown }): string {
  return `candidate_${fingerprintExperienceValue({ team: candidate.team, tree: candidate.tree })}`;
}

/** Frozen sources are part of the evaluation protocol and must invalidate stale observations. */
export function getSourceFixtureObservationFingerprint(sources: unknown[]): string {
  return `sources_${fingerprintExperienceValue(sources)}`;
}
...
export async function runExperiencePipeline(options: PipelineOptions = {}) {
  ...
  const sourceFixtureFp = getSourceFixtureObservationFingerprint(sources);
