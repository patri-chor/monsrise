  ...
  public static runSingleRound(
    ...
    for (const u of state.deployedUnits) {
      ...
      const placed: PlacedMonster = {
        ...
        gridX: u.originalX,
        gridY: u.originalY,
        initialGridX: u.originalX,
        initialGridY: u.originalY,
      ...
      // 关键：应用徽章被动属性修改（与 GameEngine.placeMonster / resetBoardForNextRound 完全一致）
      badgeOnPlace(placed);
    ...
    for (const act of state.pendingActions) {
      ...
      if (isLegalZone && isCellFree && remainingBudget >= cost) {
        ...
        rejectedActions.push({
          side: act.side,
          monsterId: act.monsterId,
          reason: !isLegalZone ? 'out_of_zone' : !isCellFree ? 'collision' : 'budget_exceeded',
        });
      }
    }
    ...
    const d1 = gameEngine.p1Score - s1Before;
    const d2 = gameEngine.p2Score - s2Before;
    const rw: 1 | 2 | 0 = d1 === d2 ? 0 : d1 > d2 ? 1 : 2;
